...
cont.1 = generate_cont_indices(mod.1)
cont.2 = generate_cont_indices(mod.2)
plot_cont_indices(cont.1,species = "A1",add_observed_means = T)
plot_cont_indices(cont.2,species = "A2",add_observed_means = T)


strat.1 = generate_strata_indices(mod.1)
strat.2 = generate_strata_indices(mod.2)
...
obs.1.mean = extr.obs.means(mod.1,modname = "A1")


obs.2.mean = extr.obs.means(mod.2,modname = "A2")
...
## But the Magnitude of the estimates depends on the approach as well
...
### A1 tends to overestimate the magnitude of the annual indices, and more so, for some strata
...
If we commpare the differences between the modeled annual mean counts (A1 and A2) and the observed mean counts, this should provide a measure of:
...
difp = ggplot(data = compdif2,aes(y = dif_lindex_obs,x = sddif,group = model,colour = model))+
  ...
  geom_label_repel(data = difdif,aes(x = sddif,y = A1,label = Region),size = 2,inherit.aes = F)+
...
### The differences between the two approaches are partly a function of the relationship between the overall observer-route variance and the stratum-level variance
...
## Either way (A1 or A2), the stratum-level trends are the same, but the Continental trends are not!
