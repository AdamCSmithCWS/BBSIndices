...
library(dplyr)
library(ggrepel)
load(paste0("data/",species," jags_models.RData"))
...
dif.t = t.strat %>% group_by(Region) %>% summarise(difference = diff(Trend))

dif.tci = t.strat %>% group_by(Region) %>% summarise(difference = diff(CIWidth_Trend))
```
...
dift = ggplot(data = dif.t, aes(x = Region,y = difference))+
  ...
  theme(axis.text.x = element_text(angle = 90))+
  labs(x = "",y = "Difference in trend estimates",title = "Difference in trends and CI-width of trends using A1 and A2 calculations for annual indices",
       subtitle = "Continental trends vary by method but stratum-level trends are the same in magnitude and uncertainty")+
